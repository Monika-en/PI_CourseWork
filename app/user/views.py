...
@app.route('/add-favourite/<company_id>')
@login_required
def add_favourite(company_id):
    favourite = Favourite(user_id=current_user.id, company_id=company_id)
    db.session.add(favourite)
    db.session.commit()
    return redirect(url_for('favorites'))

@app.route('/remove-favourite/<company_id>')
@login_required
def remove_favorite(company_id):
    favourite = Favourite.query.filter_by(user_id=current_user.id, company_id=company_id).first()
...
def favourites():
    ...
    return render_template('user/favourites/favourites.html', favorites=favourites)